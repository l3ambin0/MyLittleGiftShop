...
public class FrShowProducts extends Fragment {
    DBHelper dbH;
    ...
    private String ids_to_retrieve;
    ...
    public void fillWithDB() throws SQLException {
        ...
        for (cat_products pr : mList){
            ProductCard prCard = new ProductCard();
            prCard.setTitle(pr.getName());
            prCard.setDescription(pr.getDescription());
            prCard.setPrice(pr.getPrice());
            prCard.setImage(pr.getImage());
            prCard.setId(pr.getId());
            lista.add(prCard);
        }
